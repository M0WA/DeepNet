...
std::string PostgreSQLInsertOrUpdateStatement::UpdateOrInsertByUniqueKeys( DatabaseConnection* db ) const {
	...
	*/
	...
	std::vector<std::string>
		...
		whereInsertColumnNames, insertColumnNames,
		insertSelectColumnValues;
	...
	for(;iCols != cols.end();++iCols) {
		...
		bool allowNullColumn = pColDef->HasDefaultValue() || pColDef->IsAutoGenerated() || pColDef->IsNullable();
		...
		insertColumnNames.push_back(curColName);
		...
		if(orgStatement.IsSumColumn(curColName)) {
			...
			if(!pCurCol->IsNull()) {
				...
				setNewValuesColumnNames.push_back(curColName+"=NULL");
				insertSelectColumnValues.push_back("NULL"); }
	...
	size_t keysProcessed(ProcessCombinedUniqueKeys(whereNewValuesCombinedKeys,"m.","nvu.",false));
	...
	keysProcessed=ProcessCombinedUniqueKeys(whereInsertColumnCombinedKeys,"","nvi.",true);
	...
	std::string newValuesColumnNamesString,
		newValuesColumnValuesString,
		setNewValuesColumnNamesString,
		whereNewValuesColumnNamesString,
		whereInsertColumnNamesString,
		insertColumnNamesString,
		insertSelectColumnValuesString;
	...
	tools::StringTools::VectorToString(insertColumnNames,insertColumnNamesString,",");
	tools::StringTools::VectorToString(insertSelectColumnValues,insertSelectColumnValuesString,",");
...
INSERT INTO " << fullQualifiedTableName << " (" << insertColumnNamesString << ") \
SELECT " << insertSelectColumnValuesString << " \
FROM vals nvi \
WHERE NOT EXISTS \
( \
	SELECT 1 \
	FROM upsert up \
	WHERE " << whereInsertColumnNamesString << " OR " << whereInsertColumnCombinedKeys << "\
) \
...
size_t PostgreSQLInsertOrUpdateStatement::ProcessCombinedUniqueKeys(std::string& whereKeys,const std::string& leftPrefix, const std::string& rightPrefix, const bool omitNullColumns) const {
	...
	for(size_t keyNo = 0; i != combinedKeys.end(); ++i,++keyNo) {
		...
		for(size_t colNo = 0;k!=i->end();++k,++colNo) {
			...
			if(!tableBase->GetConstColumnByName((*k)->GetColumnName())->IsNull()) {
				if(colNo)
					ss << " AND ";
				ss << leftPrefix<<curColName<<"="<<rightPrefix<<curColName;
			}
			else if(!omitNullColumns) {
				if(colNo)
					ss << " AND ";
				ss << leftPrefix<<curColName<<"=NULL";
			}
		}
		ss << " ) ";
	}
