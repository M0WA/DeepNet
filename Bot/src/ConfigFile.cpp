...
bool ConfigFile::ParseConfigFile(const std::string& fileName, std::vector<ConfigEntry>& registeredParams)
{
	...
	if(!tools::FileTools::ReadFile(fileName, fileContent)){
		log::Logging::LogError("could not open config file: %s", fileName.c_str());
		return false; }

	if(!fileContent.size()) {
		log::Logging::LogWarn("ignoring empty config file: %s", fileName.c_str());
		return true;}
...
bool ConfigFile::ParseFile(std::vector<std::string>& fileContent, std::vector<ConfigEntry>& registeredParams) {
	...
	for(int nLine = 1; iterLines != fileContent.end(); iterLines++,nLine++)
	{
		...
		if( (equalSign = iterLines->find('=')) == std::string::npos ){
			log::Logging::LogError("error in line %d, '=' is missing: %s", nLine, iterLines->c_str());
			continue;}
		...
		//trim(key);
		if(key.empty()){
			log::Logging::LogError("error in line %d, key is empty or invalid: %s", nLine, iterLines->c_str());
			continue;}

		size_t posValue = equalSign+1;
		value = iterLines->substr(posValue,iterLines->length() - posValue);
		if(value.empty()){
			log::Logging::LogInfo("error in line %d, empty value specified for key: %s", nLine, key.c_str());}
		...
		if(iFind == registeredParams.end()) {
			log::Logging::LogWarn("ignoring unknown parameter from config file: %s",key.c_str()); }
		else {
